...
public class Player {
    ...
    public Player(Texture img, Texture img_bullet) {
        ...
        sprite.setColor(98 / 255f, 222 / 255f, 109 / 255f, 255 / 255f);
        sprite.setScale(SCALE);
        position = new Vector2(
            sprite.getWidth() * 7,
            (sprite.getScaleY() * sprite.getHeight()) / 2 + 100
        );
        bullets = new ArrayList<>();
        bulletTexture = img_bullet;

        // sound
        shootSound = Gdx.audio.newSound(
    ...
    public void update(float deltaTime) {
        ...
        if (
            ...
            bullets.add(
                new Bullet(bulletTexture, position.x + 3, position.y, 5)
            );
            bullet_delay = 20;
